...
public class PlayerController : MonoBehaviour {
  ...
  public void Die() {
    ...
    if (gameOverPrefab != null && canvasTransform != null) {
      ...
      GameOverManager gameOverManager =
          gameOverInstance.GetComponentInChildren<GameOverManager>();
      if (gameOverManager != null) {
        gameOverManager.ShowGameOver();
      } else {
        Debug.LogError(
            "O prefab de Game Over (e seus filhos) não possui o script GameOverManager!");
      }
    } else {
      Debug.LogError(
          "O Prefab de Game Over ou o Canvas Transform não foram atribuídos no Inspector do Player!");
    }
  }
  ...
  void Update() {
    ...
    if ((Input.GetKeyDown(Controls.Grab) || MobileInput.GetHeld("Grab")) &&
        canAttach)
      TryAttachToRope();
    HandleTimers();
    HandleInputs();
  ...
  private void HandleMovement() {
    if (isRolling || isAttacking || isSwinging)
      return;
    float moveInput =
        (Input.GetKey(Controls.Right) || MobileInput.GetHeld("Right")) ? 1f
        : (Input.GetKey(Controls.Left) || MobileInput.GetHeld("Left")) ? -1f
                                                                       : 0f;
  ...
  private void HandleRoll() {
    if (isGrounded &&
        (Input.GetKeyDown(Controls.Roll) || MobileInput.GetHeld("Roll")) &&
        !isRolling) {
    ...
    if ((Input.GetKeyDown(Controls.Punch) || MobileInput.GetHeld("Punch")) &&
      ...
      if (Mathf.Abs(rb.linearVelocity.x) > 0.1f) {
        ...
        StartCoroutine(PerformAttack(uppercutHitbox, "UpPunch"));
               ...
               (Input.GetKeyDown(Controls.Kick) ||
                MobileInput.GetHeld("Kick")) &&
               (Input.GetKey(Controls.Down) || MobileInput.GetHeld("Down"))) {
      StartCoroutine(PerformStomp());
    } else if (Input.GetKeyDown(Controls.Punch) ||
               MobileInput.GetHeld("Punch")) {
      if (!isGrounded)
  ...
  IEnumerator PerformAttack(GameObject hitbox, string animationName) {
    ...
    Collider2D[] hits =
        Physics2D.OverlapBoxAll(hitbox.transform.position,
                                hitbox.GetComponent<BoxCollider2D>().size, 0);
  ...
  IEnumerator PerformRoll() {
    ...
    PlayAnimation("Roll");
    boxCollider.size =
        new Vector2(originalColliderSize.x, originalColliderSize.y / 2);
    boxCollider.offset =
        new Vector2(originalColliderOffset.x,
                    originalColliderOffset.y - originalColliderSize.y / 4);
    float speed = rollDistance / rollDuration;
    rb.linearVelocity =
        new Vector2(transform.localScale.x * speed, rb.linearVelocity.y);
  ...
  void TryAttachToRope() {
    var cols = Physics2D.OverlapCircleAll(ropeDetectionPoint.position,
                                          detectionRadius);
  ...
  void AttachToRope(Rigidbody2D ropeSegment) {
    ...
    ropeHinge.anchor =
        transform.InverseTransformPoint(ropeDetectionPoint.position);
  ...
  void DetachFromRope() {
    ...
    rb.linearVelocity = new Vector2(rb.linearVelocity.x,
                                    rb.linearVelocity.y + releaseJumpBoost);
  }
}